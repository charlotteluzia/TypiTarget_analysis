...
vars$cat_rt_max      <- 1500   # consider only RTs slower than this. 
...
mean_RT    <- prep_data %>% filter(category=="target") %>% filter(target_resp==1) %>%   
                summarize(mean_RT = mean(RT))
RT_targets <- prep_data %>% filter(category=="target") %>% filter(target_resp==1)
plot_RT_targets <- ggplot(RT_targets, aes(x=RT)) + geom_density(fill="#69b3a2", color="#e9ecef", alpha=0.8)
print(plot_RT_targets)
plot_RT_targets_scatter <- ggplot(RT_targets, aes(x=RT, y=)) + geom_histogram(binwidth = 20, fill = "skyblue", color = "black")
print(plot_RT_targets_scatter)
```
