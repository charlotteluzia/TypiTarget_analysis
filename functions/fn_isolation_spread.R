fn_isolation_spread <- function(all_data, vars){
  ...
  summary_iso_spread <- all_data %>%
    ...
    filter(iso_spread == 1) %>%
    group_by(participant, typicality, category) %>%
    summarize(
    ...
    mutate(
      mem_dprime_iso      = qnorm(mem_hitr) - qnorm(mem_falr),
      mem_crit_iso        = -0.5 * (qnorm(mem_hitr) + qnorm(mem_falr))
    )
  ...
  summary_no_iso_spread <- all_data %>%
    ...
    mutate(
      mem_dprime_no_iso      = qnorm(mem_hitr) - qnorm(mem_falr),
      mem_crit_no_iso        = -0.5 * (qnorm(mem_hitr) + qnorm(mem_falr))
    )
